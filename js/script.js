window.addEventListener("DOMContentLoaded", () => {
    ...
    new MenuCard(
        ...
        9,
        '.menu .container',
        'menu__item'  // передача нового класса. С использованием rest все последующие аргументы также будут передаваться в один массив
    ...
    new MenuCard(
        ...
        9,
        '.menu .container',
        'menu__item'     // без точки, тк используем его в classList
    ...
    forms.forEach(item => { // перебираем все наши формы, тк они идут в массиве item - это наша форма 
        postData(item);     // вызываем функцию postData с каждой из форм по очереди
    });
    ...
    function postData(form) {
        form.addEventListener('submit', (e) => {
            ...
            const object = {};
            formData.forEach(function(value, key){   // тут мы перебираем данные из формата форм даты и переносим их в обычный объект
                object[key] = value;
            });
            ...
            fetch('server.php', {
                ...
                body: JSON.stringify(object) // далее, преобразуем в формат json
            })
            .then(data => data.text())
            .then(data => {
                console.log(data);
